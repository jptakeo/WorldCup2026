...
if __name__ == "__main__":
    ...
    if actual_results['home_real'].isna().sum() == actual_results.shape[0]:
        ...
        if ((actual_results.loc[actual_results['stage'] == 'semi_final', 'home_real'].isna().sum() == 0) & 
            ...
            matches_to_pred = actual_results.loc[actual_results['stage'].isin(['final'])]
            ...
            stage = 'round_of_16'
            colunas_para_substituir = ["quarterfinals", "semifinals", "final", "champion"]
            ...
            stage = 'round_of_32'
            colunas_para_substituir = ["round_of_16", "quarterfinals", "semifinals", "final", "champion"]
        ...
        if stage == 'final':
            pred_third_place = actual_results.loc[actual_results['stage'].isin(['third_place'])]
            pred_third_place["home_team"] = pred_third_place["home_team"].map(TEAM_MAP_PT_TO_EN)
            pred_third_place["away_team"] = pred_third_place["away_team"].map(TEAM_MAP_PT_TO_EN)
            ...
            probs_third_place, _, df_matches_third_place = simular_fase_e_restante(draws_26, times_26, pred_third_place)
            ...
            df_previous_summary = df_previous_summary[df_previous_summary.index.isin(df_summary.index)]
            ...
            # order based on the champion column
            df_previous_summary = df_previous_summary.sort_values(by='champion', ascending=False)
            ...
            df_matches_third_place.to_csv(f'data/probs_third_place.csv', index=False)
            df_matches_third_place.to_csv(f'docs/csv/previsoes/probs_third_place.csv', index=False)
        else:
            probs, df_summary, df_matches = simular_fase_e_restante(draws_26, times_26, matches_to_pred)
            df_previous_summary = pd.read_csv('data/summary.csv')
            # 2. Temporarily set 'team' as the index for both dataframes so they align perfectly
