...
def remover_selecoes_sem_volume(df, min_jogos=20, col_home='home_team', col_away='away_team'):
    # 1. Conta quantas vezes cada time jogou em casa e fora
    jogos_casa = df[col_home].value_counts()
    jogos_fora = df[col_away].value_counts()

    # 2. Soma as duas contagens (fill_value=0 garante que times que só jogaram fora/casa não deem erro)
    total_jogos = jogos_casa.add(jogos_fora, fill_value=0)

    # 3. Identifica as seleções que atingiram a nota de corte
    selecoes_validas = total_jogos[total_jogos > min_jogos].index

    # 4. Filtra o DataFrame original (exige que AMBOS os times da partida sejam válidos)
    df_limpo = df[
        df[col_home].isin(selecoes_validas) &
        df[col_away].isin(selecoes_validas)
    ].copy()

    # Ordena a lista de times para garantir que o mapeamento no Stan seja alfabético
...
def obter_fator_i(row):
    ...
    if 'fifa world cup' in t and 'qualification' not in t:
        return 60
    continental_comps = ['uefa euro', 'copa américa', 'african cup of nations',
...
def preparar_dados_ciclo(caminho_csv, data_inicio, data_fim=None, min_jogos=20, aplicar_decaimento=True, decay_alpha=0.001):
    """
    ...
    # Filtro temporal do ciclo
    filtro = (df['date'] >= data_inicio)
    if data_fim:
        filtro &= (df['date'] < data_fim)
    df_cycle = df[filtro].copy()

    # Filtragem de seleções com volume mínimo de jogos
    counts = df_cycle['home_team'].value_counts().add(
        df_cycle['away_team'].value_counts(), fill_value=0)
    teams_validos = counts[counts >= min_jogos].index
    df_cycle = df_cycle[df_cycle['home_team'].isin(
        teams_validos) & df_cycle['away_team'].isin(teams_validos)].copy()

    # Cálculo da importância (fator_i)
    df_cycle['fator_i'] = df_cycle.apply(obter_fator_i, axis=1)

    # Aplicação do decaimento exponencial (ou apenas o peso base)
    if aplicar_decaimento:
        max_date = df_cycle['date'].max()
        df_cycle['days_ago'] = (max_date - df_cycle['date']).dt.days
        df_cycle['game_weight'] = (
            df_cycle['fator_i'] / 10) * np.exp(-decay_alpha * df_cycle['days_ago'])
    else:
        df_cycle['game_weight'] = (df_cycle['fator_i'] / 10.0)

    # Mapeamento oficial de times (inteiros para o Stan)
...
def carregar_priors_ranking(caminho_ranking, teams_list):
    ...
    team_col = next((c for c in cols if 'sele' in c.lower()
                    or 'team' in c.lower()), cols[min(1, len(cols)-1)])
    pts_col = next((c for c in cols if 'pont' in c.lower()
                   or 'pts' in c.lower() or 'point' in c.lower()), cols[-1])

    fifa_ranking_data = pd.Series(
        df_ranking[pts_col].values, index=df_ranking[team_col]).to_dict()
    all_pts = [float(str(v).replace(',', '.'))
               for v in fifa_ranking_data.values() if pd.notnull(v)]
    ...
    for team in teams_list:
        # Se o time não estiver no ranking, recebe a média
        pts = fifa_ranking_data.get(team, mean_pts)
        normalized_val = (
            float(str(pts).replace(',', '.')) - mean_pts) / std_pts
        team_prior_strength.append(normalized_val)

    print(
        f"Sucesso! Posições do ranking {caminho_ranking} carregadas e normalizadas.")
